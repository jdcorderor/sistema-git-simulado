...
class Repositorio:
    ...
    def __init__(self):
        self.repo_commits = None # Atributo inicializado "None" debido a que no se ha ejecutado un Commit.
    ...
    def git_commit(self, identificador: str):
        ultimo_commit = self.rama_activa.get_commit_reciente() # Variable que almacena el último Commit registrado en la Rama Activa.
        lista = self.listar_archivos_commit() # Invoca al método auxiliar "listar_archivos_commit".
        if self.rama_activa.commit_reciente != None:
        ...
        print("\nCommit '%s' done. Files:"%(identificador), end=" ")
        self.imprimir_nombres_archivos()
    ...
    def git_branch_create(self, identificador: str):
        rama_nueva = Rama(identificador, self.rama_activa.get_commit_reciente()) # Construye el objeto del tipo de la clase Rama. Invoca al getter del Commit más reciente ("commit_reciente") de la Rama Activa.
        self.repo_ramas.append(rama_nueva) # Agrega el objeto creado a la lista de Ramas existentes ("repo_ramas").
        print("Branch '%s' created succesfully"%(identificador))
    ...
    def git_switch(self, rama_objetivo: str):
        ...
        print("Switched to '%s' branch succesfully"%(rama_objetivo))

    # Método encargado de fusionar (Merge) dos Ramas del Repositorio:
    def git_merge(self, rama_merge: str):
        index_rama_merge = self.index_ramas(rama_merge) # Invoca al método auxiliar "index_ramas" y almacena el índice (dentro de la lista "repo_ramas") de la rama a fusionar con la Rama Activa.
        merge_commit_id = f"Merge {rama_merge} into {self.rama_activa.get_nombre_rama()}" # Crea el mensaje del Commit asociado al Merge.
        lista_archivos_merge = self.rama_activa.get_commit_reciente().get_lista_archivos() + self.repo_ramas[index_rama_merge].get_commit_reciente().get_lista_archivos() # Fusiona las listas de Archivos de los Commits más recientes de ambas Ramas. 
        self.rama_activa.set_commit_reciente(Commit(merge_commit_id, lista_archivos_merge, self.rama_activa.get_commit_reciente())) # Actualiza el Commit más reciente de la Rama Activa.
        self.repo_ramas[index_rama_merge].set_commit_reciente(Commit(merge_commit_id, lista_archivos_merge, self.repo_ramas[index_rama_merge].get_commit_reciente())) # Actualiza el Commit más reciente de la Rama fusionada.
        self.repo_commits.append(self.rama_activa.get_commit_reciente()) # Agrega el Commit (asociado al Merge) al historial de Commits ("repo_commits").
        print("\n\n'%s' completed. Files:"%(merge_commit_id), end=" ")
        self.imprimir_nombres_archivos()
    ...
    def git_log(self):
        ...
        while (i >= 0):
            print("%s"%(self.repo_commits[i].get_commit_id())) # Ciclo que imprime el "commit_id" de todos los objetos del tipo de la clase Commit registrados en el historial ("repo_commits").
            i -= 1
        print("\n")
    ...
    def git_branch(self):
        ...
        while (i >= 0):
            print("%s"%(self.repo_ramas[i].get_nombre_rama())) # Ciclo que imprime el "nombre_rama" de todos los objetos del tipo de la clase Rama registrados en "repo_ramas".
            i -= 1
        print("\n")
    ...
    def index_ramas(self, clave_rama: str):
        for i in range (len(self.repo_ramas)):
            if self.repo_ramas[i].get_nombre_rama() == clave_rama: # Recorre los elementos de la lista del tipo de la clase Rama en busca de una coincidencia del atributo "nombre_rama" con la cadena "clave_rama".
    ...
    # Método auxiliar para imprimir los nombres de los objetos de la clase Archivo:
    def imprimir_nombres_archivos(self):
        nombres_archivos = set()
        for commit in self.repo_commits:
            for archivo in commit.get_lista_archivos():
                nombres_archivos.add(archivo.get_nombre_archivo())
